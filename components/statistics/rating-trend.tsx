...
export function RatingTrend({ points }: { points: readonly RatingTrendPoint[] }) {
  if (points.length === 0) {
    return <p className="border-l-4 border-slate-200 bg-white px-4 py-4 text-sm text-slate-600">暂无带 Rating 的训练记录。</p>;
  }
        ...
        <div className="flex h-52 min-w-max items-end gap-3 border-b border-slate-300 px-1">
          {points.map((point) => (
            <div className="flex w-14 shrink-0 flex-col items-center justify-end" key={point.date}>
              ...
              <div
                aria-label={`${point.date} 平均 Rating ${point.averageRating}，${point.problemCount} 道题`}
                ...
                title={`${point.problemCount} problem${point.problemCount === 1 ? "" : "s"}`}
              />
